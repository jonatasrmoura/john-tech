...
export const ServicesSection = memo(function ServicesSection({  }: ServicesSectionProps) {
  return (
    <section id="servicos" className="px-4 py-20 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-7xl">
            <div className="mb-12 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
              <div>
                <p className="text-sm font-semibold uppercase tracking-[0.3em] text-cyan-300">Serviços</p>
                <h2 className="mt-3 text-3xl font-semibold text-white sm:text-4xl">Soluções modernas para notebook, desktop e home office</h2>
              </div>
              <a href="#contato" className="inline-flex items-center gap-2 text-sm font-semibold text-cyan-200 transition hover:text-cyan-100">
                Solicitar um orçamento personalizado <Icons.ChevronRight size={16} />
              </a>
            </div>
                ...
                return (
                  <motion.article key={service.title} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.2 }} transition={{ duration: 0.45, delay: index * 0.06 }} className="group rounded-[1.75rem] border border-white/10 bg-white/5 p-6 shadow-[0_0_40px_rgba(0,0,0,0.25)] backdrop-blur-xl transition hover:-translate-y-1 hover:border-cyan-400/30 hover:bg-cyan-400/10">
                    ...
                    <div className="mt-6 inline-flex items-center gap-2 text-sm font-semibold text-cyan-200">
                      Solicitar agora <Icons.ArrowRight size={15} />
                    </div>
                  </motion.article>
                );
              })}
